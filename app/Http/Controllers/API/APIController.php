        ...
        if ($request->isMethod('post')) { // Check if the incoming HTTP Request Method/Verb is 'POST'
            ...
            if (\Illuminate\Support\Facades\Auth::attempt(['email' => $userData['email'], 'password' => $userData['password']])) {
                ...
                // dd($user);
                                                                                       ...
                                                                                       // Generate a new Access Token using "Laravel Sanctum" Package for the newly registered user
                $accessToken = $user->createToken($userData['email'])->plainTextToken; // Utilizing the HasApiTokens trait we're using in the User.php model    // Managing Personal Access Tokens: https://laravel.com/docs/9.x/sanctum#issuing-api-tokens
                                                                                       // dd($accessToken);

                // UPDATE the `access_token` column in `users` table with the new $accessToken value (which is generated using "Laravel Sanctum" package)
                User::where('email', $userData['email'])->update(['access_token' => $accessToken]);

                                          // Send back the newly submitted user data of the successful operation to the user again! (Note: This a well-known convention of APIs to send back the successful accepted data!)
                return response()->json([ // Note: $user is AUTOMATICALLY converted from Laravel/PHP object to JSON! But we'll manually convert it to JSON using    return response()->json()    to show the "users" JSON property key/name in the response! Try the TWO cases and check the difference in Postman in "Pretty" tab!    // JSON Responses: https://laravel.com/docs/9.x/responses#json-responses
                    'status'  => true,
                    'message' => 'User registered successfully! (using Sanctum)',
                    // 'token'   => $apiToken,
                    'token'   => $accessToken,
                    'user'    => $user,
                ], 201); // 201 HTTP Status Code: Created    // With POST requests, 201 Created HTTP Status Code is sent with the HTTP Response
        ...
        if ($request->isMethod('post')) { // Check if the incoming HTTP Request Method/Verb is 'POST'
            ...
            if (\Illuminate\Support\Facades\Auth::attempt(['email' => $userData['email'], 'password' => $userData['password']])) {
                ...
                // dd($user);
                                                                                       ...
                                                                                       // Generate a new Access Token using "Laravel Sanctum" Package for the newly registered user, and save it in the `access_token` column in `users` table (and overwrite/override/replace the old Access Token in `access_token` column if there is any!)
                $accessToken = $user->createToken($userData['email'])->plainTextToken; // Utilizing the HasApiTokens trait we're using in the User.php model    // Managing Personal Access Tokens: https://laravel.com/docs/9.x/sanctum#personal-access-tokens
                                                                                       // dd($accessToken);
                ...
                return response()->json([ // Note: $user is AUTOMATICALLY converted from Laravel/PHP object to JSON! But we'll manually convert it to JSON using    return response()->json()    to show the "users" JSON property key/name in the response! Try the TWO cases and check the difference in Postman in "Pretty" tab!    // JSON Responses: https://laravel.com/docs/9.x/responses#json-responses
                    'status'  => true,
                    'message' => 'User logged in successfully! (using Sanctum)',
                    'token'   => $accessToken,
                    'user'    => $user,
                ], 201); // 201 HTTP Status Code: Created    // With POST requests, 201 Created HTTP Status Code is sent with the HTTP Response
